...
/**
 * Exposes the system's dynamic Material You palette (Android 12+, extracted
 * from the wallpaper by Android itself via the `system_accent1_x` /
 * `system_neutral2_x` tones) to the webview, so the app's theme follows the
 * device wallpaper instead of a fixed color. Ported from Orbit/Mago's
 * DynamicColorPlugin so a future widget can read the exact same values —
 * app and widget would then never be able to diverge.
 */
